...
"@
    Set-Content -Path (Join-Path $deployFolder 'Deploy.ps1') -Value $deployScript -Encoding ascii
...
"@
    ...
    Invoke-ExternalTool -FilePath 'MakeWinPEMedia' -ArgumentList @('/ISO', $winPEWorkDir, $deployISOPath) -Description "MakeWinPEMedia failed to create WinPE Deploy ISO at '$deployISOPath'"
